...
print("[LibasJoiner] Iniciado com Auto Joiner funcional.")
...
-- CONFIG WEBSOCKET
--------------------------------------------------------
local wsURL = "wss://blast-garden-passion-month.trycloudflare.com" -- URL do WS do Fly.io

-- Detectar a biblioteca WebSocket suportada pelo exploit
local wsLib = nil
...
elseif WebSocket and WebSocket.connect then
    ...
    warn("[LibasFinder] ❌ Seu exploit NÃO possui WebSocket (websocket/WebSocket).")
    return
end
...
local function conectarWS()
    print("[LibasFinder] Conectando ao WebSocket...")

    -- Tenta conectar via pcall
    local ok, socket = pcall(function()
        return wsLib(wsURL)
    end)

    if not ok or not socket then
        warn("[LibasFinder] Falha ao conectar. Tentando novamente em 5s...")
        task.wait(5)
        return conectarWS()
    end

    ws = socket
    print("[LibasFinder] 🟢 WebSocket conectado!")
    ...
    ws.OnMessage:Connect(function(msg)
		local ok, err = pcall(function()
			-------------------------------------------------
			-- TODO O TEU CÓDIGO DO CALLBACK AQUI DENTRO
			-------------------------------------------------

			print("[LibasFinder] Mensagem recebida:", msg)
			...
			if not okDecode or type(data) ~= "table" then
				warn("[LibasFinder] JSON inválido:", msg)
				return
			end

			if data.type == "server_update" and data.server then
				local s = data.server
				if s.jobId and tonumber(s.timestamp) > startTimestamp then
					criarBotao(s)
				end
			end
		end)
		...
		if not ok then
			warn("[LibasFinder] ERRO NO CALLBACK DO WEBSOCKET:", err)
		end
	end)
    ...
    ws.OnClose:Connect(function()
        warn("[LibasFinder] 🔴 WebSocket desconectado. Tentando reconectar em 2s...")
        task.wait(2)
        conectarWS()
    end)
end
...
print("[LibasJoiner] Auto Joiner ativo com sistema de filtros e valor mínimo.")
